...
def _create_demand_chart(df_processed, power_col):
    ...
    fig.add_trace(go.Scatter(
        ...
        y=df_processed[power_col],
                ...
                st.success("✅ Basic forecast completed successfully")
                ...
                st.download_button(
                    ...
                    data=csv_data,
                    file_name="simple_forecast.csv",
                    mime="text/csv"
                )

            # Basic power statistics("#### 📊 Analysis Complete")
                st.success("✅ Basic forecast completed successfully")
                ...
                st.download_button(
                    ...
                    file_name="simple_forecast.csv",
                    mime="text/csv"
                )mple forecast table with {len(forecast_df)} sample points")
...
if uploaded_file:
    try:
        ...
        if timestamp_col and power_col:
            ...
            with col1:
                n_anchors = st.number_input("Number of sample points", min_value=1, max_value=10, value=5, step=1)
            
            with col2:
                horizons = [st.selectbox("Forecast horizon (min)", [1, 5, 10, 20], index=0)]

            # Simple anchor preparation
            anchor_df = roc_df.dropna(subset=["ROC (kW/min)"]).head(20).copy()
            anchor_df = anchor_df.reset_index(drop=True)

            # Simple anchor sampling - just use first few points
            anchor_indices = list(range(min(n_anchors, len(anchor_df))))
            
            # Simple forecast table - just first few points
            if len(anchor_indices) == 0:
                st.warning("⚠️ Data structure not supported")
            else:
                # Build simple forecast table with limited data
                results = []
                for idx in anchor_indices[:5]:  # Limit to first 5 points
                    anchor_row = anchor_df.iloc[idx]
                    anchor_ts = anchor_row["Timestamp"]
                    P_now = anchor_row["Power (kW)"]
                    ROC_now = anchor_row["ROC (kW/min)"]
                    
                    # Just use first horizon to avoid loops
                ...
                st.info(f"📋 Simple forecast table with {len(forecast_df)} sample points")
                st.dataframe(forecast_df, use_container_width=True)
        ...
        st.warning("⚠️ Data structure not supported - please check your file format")
else:
